...
#define VERSION 'j'
#define DEVICE_NAME "Land Rover Evoque (Diesel) V1.0" //$CLUSTERMODEL $SKETCH_VERSION
...
#define LED_BUILTIN 2
  ...
  #define MCPH_EN_PIN 10 // Modify only when using arduino
#endif
#define DEVICE_UNIQUE_ID "3uvyag1d-fi05-2my8-9pvt-1ofj-0lbbb2559vhv" // Generate ids randomly. Get a random ID from https://duid.xconnect.cc changeme-pret-typl-ease-hehe-iamatempid12
...
void opSendH(short addr, byte a, byte b, byte c, byte d, byte e, byte f, byte g, byte h) {
  unsigned char data[8] = { a, b, c, d, e, f, g, h };
  CANH.sendMsgBuf(addr, 0, 8, data);
}
...
void setup() {	
  ...
  CAN_INIT_H:
    if (CANH.begin(MCP_ANY, CAN_500KBPS, MCP_16MHZ) == CAN_OK) {
      FlowSerialDebugPrintLn("CAN BUS HIGH CONTROLLER initiation ok");
      pulseLED(50);
      delay(100);
      pulseLED(50);
    } else {
      pulseLED(50);
      FlowSerialDebugPrintLn("CAN BUS HIGH CONTROLLER initiation failed, retrying in 500ms");
      delay(100);
      pulseLED(400);
      goto CAN_INIT_H;
    }
    // Set CAN BUS mode to MCP_ANY
    CANH.setMode(MCP_ANY);
  }

// --- Loop function, runs continuously
void loop() {
	shCustomProtocol.loop();
  vTaskDelay(1); // Delay for the MCB
	// Wait for data
